...
// Utils to map data between snake and cammel cases. Taken from Objection.js (v 2.1.3)
// Source: https://github.com/Vincit/objection.js/blob/master/lib/utils/identifierMapping.js

function isDigit(char: any): any {
...
function isAllUpperCaseSnakeCase(str: any): any {
...
function camelCase(str: any, { upperCase = false } = {}): any {
  if (str.length === 0) {
    return str;
  }
...
function mapLastPart(mapper: any, separator: any) {
  return (str: any): any => {
...
function knexIdentifierMappers({ parse, format, idSeparator = ':' }: any = {}): any {
  ...
  return {
    wrapIdentifier(identifier: any, origWrap: any): any {
...
export function knexSnakeCaseMappers(opt = {}): any {
  return knexIdentifierMappers({
    parse: (str: any) => camelCase(str, opt),
    format: (str: any) => snakeCase(str, opt),
  });
}
